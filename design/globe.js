/* historya-globe / historya-outline
   Vanilla custom elements — sin framework, listos para envolver en un componente Vue.
   Geometría real: Natural Earth 110m (world-atlas), proyección ortográfica d3-geo,
   render en canvas (una sola capa, sin DOM por país) para que el arrastre no cueste nada.
   Requiere d3 y topojson-client cargados antes (tags fijados en <helmet>). */
(function () {
  'use strict';
  ...
  function libs() {
    return new Promise(function (res) {
      (function t() { (window.d3 && window.topojson) ? res() : setTimeout(t, 40); })();
    });
  }
  ...
  function world() {
    if (!worldP) {
      worldP = libs()
        .then(function () { return fetch(ATLAS); })
  ...
  function esName(f) {
    var d = window.HISTORYA_ES_NAMES || {};
    return d[f.properties.name] || f.properties.name;
  }
  ...
  class HistoryaGlobe extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.bind();
      window.HISTORYA_GLOBE = this;
      world().then(function (f) {
        this.feats = f;
        this.resize();
        if (window.HISTORYA_ON_READY) window.HISTORYA_ON_READY(f.length);
      }.bind(this));
    }
    ...
    resize() {
      ...
      this.proj = window.d3.geoOrthographic().clipAngle(90).translate([w / 2, h / 2]);
      this.path = window.d3.geoPath(this.proj, this.ctx);
      this.grat = window.d3.geoGraticule10();
      this.draw();
      this.loop();
    }
    ...
    bind() {
      ...
      el.addEventListener('pointermove', function (e) {
        ...
        requestAnimationFrame(function () {
          ...
          if (f !== self.hover) {
            self.hover = f;
            if (window.HISTORYA_ON_HOVER) window.HISTORYA_ON_HOVER(f ? { name: f.__es, en: f.properties.name, id: String(f.id) } : null);
            self.loop(); self.draw();
          }
        });
      });
      ...
      el.addEventListener('pointerleave', function () {
        if (down) return;
        if (self.hover) { self.hover = null; if (window.HISTORYA_ON_HOVER) window.HISTORYA_ON_HOVER(null); self.draw(); }
      });
    ...
    pick(f) {
      this.sel = f;
      this.hover = null;
      if (window.HISTORYA_ON_FOCUS) window.HISTORYA_ON_FOCUS({ name: f.__es, en: f.properties.name, id: String(f.id) });
      this.flyTo(f, false);
    }
    ...
    flyTo(f, silent) {
      ...
      this.flying = {
        ...
        t0: performance.now(), dur: silent ? 480 : 760,
          ...
          if (window.HISTORYA_ON_SELECT) window.HISTORYA_ON_SELECT({ name: f.__es, en: f.properties.name, id: String(f.id) });
        }
      };
      this.loop();
    }
  ...
  class HistoryaOutline extends HTMLElement {
    ...
    render() {
      ...
      if (!f) return;
      var proj = window.d3.geoMercator().fitExtent([[2, 2], [w - 2, h - 2]], f);
      var path = window.d3.geoPath(proj, c);
      c.beginPath(); path(f);
