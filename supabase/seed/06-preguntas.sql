...
delete from public.questions where topic_id in (select id from public.topics where slug in ('absolutismo', 'africa', 'america-precolombina', 'andalus', 'bizancio', 'china-imperial', 'crisis-siglo-xiv', 'descubrimientos', 'egipto', 'entreguerras', 'espana-siglo-xx', 'estados-unidos', 'feudalismo', 'gran-guerra', 'grecia', 'guerra-fria', 'helenismo', 'ilustracion', 'imperialismo', 'india', 'industrializacion', 'islam', 'japon', 'mesopotamia', 'mundo-actual', 'plena-edad-media', 'prehistoria', 'reforma', 'renacimiento', 'revolucion-cientifica', 'revolucion-francesa', 'revolucion-rusa', 'revoluciones-liberales', 'roma-imperio', 'roma-republica', 'rumania', 'segunda-guerra'));
...
-- Pregunta: estados-unidos-1
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Cuál fue la primera colonia inglesa permanente en Norteamérica?', 'Jamestown, en Virginia, se fundó en 1607. Plymouth, la de los peregrinos del Mayflower, llegó en 1620.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
...
-- Pregunta: estados-unidos-2
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué significaba el lema «no hay impuestos sin representación»?', 'Las asambleas coloniales votaban sus propios impuestos. Las leyes de Londres tras 1763 rompieron esa costumbre.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('Que los colonos se negaban a pagar cualquier impuesto', false, 0),
  ('Que el Parlamento británico, donde los colonos no tenían diputados, no podía cobrarles impuestos', true, 1),
  ('Que solo los esclavistas debían pagar impuestos', false, 2),
  ('Que los impuestos debían votarlos los gobernadores reales', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-3
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Quién redactó la Declaración de Independencia de 1776?', 'Jefferson escribió el borrador, que el Congreso Continental retocó y aprobó el 4 de julio de 1776.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('George Washington', false, 0),
  ('Benjamin Franklin', false, 1),
  ('Thomas Jefferson', true, 2),
  ('Abraham Lincoln', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-4
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué victoria convenció a Francia de aliarse con los colonos?', 'Saratoga, en 1777, demostró que los rebeldes podían ganar. Yorktown, en 1781, fue la victoria final, con ayuda francesa.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
...
-- Pregunta: estados-unidos-5
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué es la Carta de Derechos de Estados Unidos?', 'Garantizan, entre otras, la libertad de expresión, de prensa y de religión.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('La Declaración de Independencia', false, 0),
  ('Las diez primeras enmiendas a la Constitución, aprobadas en 1791', true, 1),
  ('El tratado de paz con Gran Bretaña', false, 2),
  ('La ley que abolió la esclavitud', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-6
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿A quién compró Estados Unidos la Luisiana en 1803?', 'Jefferson pagó quince millones de dólares y el país dobló su superficie.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('A España', false, 0),
  ('A Gran Bretaña', false, 1),
  ('A la Francia de Napoleón', true, 2),
  ('A México', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-7
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué fue el Sendero de Lágrimas?', 'Fue consecuencia de la Ley de Traslado Forzoso de 1830. En la marcha de 1838-1839 murieron unos cuatro mil cheroquis.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('La ruta de los esclavos fugitivos hacia Canadá', false, 0),
  ('La deportación forzosa de los cheroquis y otros pueblos del sureste hacia el oeste', true, 1),
  ('El camino de los buscadores de oro a California', false, 2),
  ('La retirada del ejército confederado en 1865', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-8
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué perdió México en la guerra de 1846-1848?', 'Por el Tratado de Guadalupe Hidalgo, México cedió California, Nuevo México, Arizona, Nevada, Utah y parte de Colorado.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('Solo Texas', false, 0),
  ('Florida y Cuba', false, 1),
  ('Más de la mitad de su territorio, incluidas California y Nuevo México', true, 2),
  ('Nada, porque ganó la guerra', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-9
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué hecho desencadenó la secesión de los estados del sur?', 'Lincoln y el Partido Republicano se oponían a extender la esclavitud al oeste. Once estados se separaron y formaron la Confederación.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('La compra de Alaska', false, 0),
  ('La victoria electoral de Abraham Lincoln en 1860', true, 1),
  ('La guerra con México', false, 2),
  ('El asesinato de Lincoln', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-10
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué hizo la 13ª enmienda de la Constitución, de 1865?', 'La 14ª dio la ciudadanía a todos los nacidos en el país y la 15ª prohibió negar el voto por motivos de raza.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('Dar el voto a las mujeres', false, 0),
  ('Abolir la esclavitud', true, 1),
  ('Prohibir el alcohol', false, 2),
  ('Crear el impuesto sobre la renta', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-11
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué establecía la sentencia Plessy contra Ferguson de 1896?', 'Dio cobertura legal a las leyes de Jim Crow. La anuló la sentencia Brown contra la Junta de Educación en 1954.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('Que la segregación racial era constitucional si los servicios eran «separados pero iguales»', true, 0),
  ('Que los esclavos fugados debían ser devueltos a sus dueños', false, 1),
  ('Que la segregación escolar era inconstitucional', false, 2),
  ('Que los indígenas eran ciudadanos', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-12
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué territorios obtuvo Estados Unidos tras la guerra con España de 1898?', 'Cuba quedó como protectorado y se independizó en 1902; Puerto Rico y Guam siguen siendo territorios estadounidenses.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('Cuba, Puerto Rico, Guam y Filipinas', true, 0),
  ('Florida y Luisiana', false, 1),
  ('Hawái y Alaska', false, 2),
  ('Panamá y Nicaragua', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-13
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué fue el New Deal?', 'Incluyó grandes obras públicas, regulación de la banca y la bolsa, subsidio de paro y pensiones públicas.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('El tratado de paz de la Primera Guerra Mundial', false, 0),
  ('El programa de Roosevelt contra la Gran Depresión', true, 1),
  ('La ayuda estadounidense a Europa tras 1945', false, 2),
  ('La ley que acabó con la segregación', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-14
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué hizo Rosa Parks en Montgomery en 1955?', 'Su detención desencadenó el boicot de los autobuses dirigido por Martin Luther King, punto de partida del movimiento por los derechos civiles.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('Ganar las elecciones a la alcaldía', false, 0),
  ('Negarse a ceder su asiento del autobús a un pasajero blanco', true, 1),
  ('Fundar el Ku Klux Klan', false, 2),
  ('Escribir La cabaña del tío Tom', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-15
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Por qué dimitió el presidente Richard Nixon en 1974?', 'Su equipo espió la sede del Partido Demócrata y él intentó encubrirlo. Es el único presidente que ha dimitido.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('Por perder la guerra de Vietnam', false, 0),
  ('Por el escándalo Watergate', true, 1),
  ('Por una grave enfermedad', false, 2),
  ('Por la crisis del petróleo', false, 3)
) as opcion(label, is_correct, position);

-- Pregunta: estados-unidos-16
with nueva as (
  insert into public.questions (topic_id, prompt, explanation, difficulty, published)
  select id, '¿Qué ocurrió el 6 de enero de 2021?', 'Trump no reconoció su derrota en las elecciones de 2020. El asalto fue el mayor ataque a la sede del Congreso desde 1814.', 3, true
  from public.topics where slug = 'estados-unidos'
  returning id
)
insert into public.question_options (question_id, label, is_correct, position)
select nueva.id, opcion.label, opcion.is_correct, opcion.position from nueva, (values
  ('La toma de posesión de Barack Obama', false, 0),
  ('El asalto al Capitolio por seguidores de Trump para impedir la proclamación de Biden', true, 1),
  ('Los atentados de Al Qaeda', false, 2),
  ('La retirada de Afganistán', false, 3)
) as opcion(label, is_correct, position);
