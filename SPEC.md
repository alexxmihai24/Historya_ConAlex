...
- **Fase 3 — TERMINADA (16-17/09/2026):** lecciones, documentos y preguntas, tema a tema. **Los 37 temas están traducidos al rumano**, el 100 % del temario. Diseño aplicado:
  - Lecciones rumanas empaquetadas en `src/data/topics/ro/<slug>.ts`, con la misma forma que el tema español; `useLesson` las sirve cuando el idioma es rumano y cae al español si un tema no está traducido (ya no ocurre con ninguno, pero el mecanismo de respaldo se deja tal cual para un tema nuevo que se añada más adelante). Sin migración ni SQL.
  ...
  - Test de forma: mismos apartados, fechas, conceptos, debates, fuentes, preguntas y opciones que el español, mismo número de pies de imagen y las mismas notas destacadas. El número de preguntas y de conceptos varía por tema (`mesopotamia` tiene 15 preguntas, `prehistoria` 14 y ningún documento, por ser la época anterior a la escritura): el test compara contra el número real de cada tema.
  ...
  - **Estado a 17/09/2026: 37 de 37 temas traducidos, fase 3 completa.** `npm test` (11679 comprobaciones) y `npm run build` en verde. Pendiente solo de Alex: probar en producción con dos cuentas reales (B2/E1/E2) y dar su opinión sobre el tono de la traducción.
  - **Procedimiento para traducir un tema nuevo, si se añade alguno más adelante:** copiar la forma del español (mismos apartados y notas destacadas, fechas, conceptos, debates, fuentes, documentos comentados, pies de imagen en el orden de `TOPIC_IMAGES[slug]` y preguntas con las opciones en la misma posición), registrar el tema en `src/data/topics/ro/index.ts` y pasar `npm test` y `npm run build`. Ningún paso requiere migración ni SQL.
